...
export interface RgCollectionLinks {
  first: string;
  last: string;
  self: string;
  next?: string;
...
export type RgIncluded = RgKeyedResponseObjects[RgEntityType];
/**
 ...
 */
export interface RgResponseBody<K extends RgEntityType = RgEntityType> {
  meta: RgResponseMeta;
  data: Record<number, RgResponseObject<K> & RgIncluded>;
  included: RgIncluded[];
  links?: RgCollectionLinks;
}
