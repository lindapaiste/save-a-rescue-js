 ...
 *
 * original should always be defined, but large and small are occasionally null
 */
...
export type Color = HasName;

export type Pattern = HasName;
...
export interface HostedVideo {
  ...
  fileSize: number;
  /**
   * url to the video hosted on amazon aws
   */
  url: string;
  /**
   * datetime like "2020-04-08T05:52:41Z"
   */
  created: string;
  updated: string;
}
 ...
 */
export interface RgEntityAttributes {
  animals: Animal;
  breeds: Breed;
  colors: Color;
  patterns: Pattern;
  species: Species;
  statuses: Status;
  // fosters: any; // Contact; -- creates problems with attribute filter.  Not sure why.
  locations: Location;
  orgs: Organization;
...
export type RgEntityType = keyof RgEntityAttributes;

export interface RgEntityIdentifier<T extends RgEntityType = RgEntityType> {
  type: T;
  id: string;
}
