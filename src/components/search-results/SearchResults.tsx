...
import { SearchReturns } from "../../services/search/useSearchResults";
...
export const SearchNumFound = ({
  species,
  total,
}: PropSpecies & { total?: number }) => {
  if (!isDefined(total)) {
    return null;
  }
  return (
    <h2>
      ...
      <span className="pets-found">
        {speciesLabel(species, "plural")} Found
      </span>
    </h2>
  );
};
...
export const RenderSearchResults = ({
  ...
  return (
    <div className={`search-results format-${format}`}>
      ...
      <SearchNumFound total={total} species={species} />
      <ResultsComponent
        animalIds={animalIds || []}
        ...
        placement={placement}
      />
      {!isLoadedAll && !!total && <LoadNext onClick={loadNext} />}
      {isError && (
        <FetchError
