 ...
 */
export const BarRating = ({filled, total = 100, minimumFill = 0, numbered = false, children, className}: Props) => {
    ...
    return (
        <div className={combineClasses(["bar-rate", className, numbered ? "numbered" : undefined])}>
            {children}
            {numbered && (
                <span className="numbers">
                    {range(1, total + 1).map(n => (
                        <span style={{width: each + '%'}} key={n}>{n}</span>
                    ))}
                </span>
            )}
